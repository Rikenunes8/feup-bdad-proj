...
CREATE TABLE Estudante(
  ...
  turma           INTEGER NOT NULL REFERENCES Turma ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT Gender CHECK(género IN ('Masculino','Feminino','Não-Binário')),
  CONSTRAINT Room CHECK(interno IN (0,1))
);
...
CREATE TABLE NãoDocente(
  ...
  turno           INTEGER,
  CONSTRAINT Gender CHECK(género IN ('Masculino','Feminino','Não-Binário')),
  CONSTRAINT Types  CHECK(tipo IN ('Auxiliar de Limpeza', 'Auxiliar de Gestão' ,'Administrativo')),
...
CREATE TABLE Quarto(
  ...
  turnoDia    INTEGER UNIQUE NOT NULL REFERENCES NãoDocente ON DELETE RESTRICT ON UPDATE CASCADE, 
  turnoNoite  INTEGER UNIQUE NOT NULL REFERENCES NãoDocente ON DELETE RESTRICT ON UPDATE CASCADE, 
...
CREATE TABLE Clube(
  ...
  CONSTRAINT Semana       CHECK(diaSemana IN ('Segunda','Terça','Quarta','Quinta' ,'Sexta', 'Sábado', 'Domingo'))
);
